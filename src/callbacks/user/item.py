...
async def execute(bot: Bot, user: User, message_id: int, data: dict):
    ...
    add_to_cart_callback = f'{{"role": "user", "item_id": {item.id}}}addToCart'
    ...
    if item in user.cart.items:
        ...
        cart_button = ((
            (language.minus, f'{{"role": "user", "item_id": {item.id}}}removeFromCart'),
            (item_cart_quantity, f'{JSON_USER}None'),
            (language.plus, add_to_cart_callback) if item_cart_quantity <= item.amount else (language.cross, f'{JSON_USER}None')
        ))
